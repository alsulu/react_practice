...
const Error = () => {
  ...
  return (
    <div className={styles.error_cont}>
      ...
      {shower ? (
        <p className={styles.russianError}>
          Ошибка 404. Извините, страница, которую вы ищите, не существует :(
        </p>
      ) : (
        <button className={styles.buttonTranslate} onClick={handleClick}>
          Попробуйте перевести сообщение об ошибке и нажмите, чтобы проверить
          себя
        </button>
      )}
